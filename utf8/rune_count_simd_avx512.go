...
func runeCountAVX512(p []byte) (int, bool) {
	...
	continuationBytes := sums[0] + sums[1] + sums[2] + sums[3] +
		sums[4] + sums[5] + sums[6] + sums[7]
	count -= int(continuationBytes >> 7)
...
// byte sums of its expected continuation positions to continuationSums. On
// valid UTF-8 those positions are exactly the continuation bytes; an invalid
...
func validateAndCountAVX512DirtyBlock(
	...
	for range avx512WindowSize / avx512VectorSize {
		...
		sums = sums.Add(expected.SumOf8AbsDiff(zero))
